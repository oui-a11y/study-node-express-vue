...
<script>
  ...
  export default {
    ...
    computed: {
      checkAllFlag() {
        var that = this;
        return that.checkedCount == that.cartList.length;
      },
      checkedCount() {
        var that = this;
        var num = 0;
    ...
    methods: {
      ...
      delCart() {
        var that = this;
        axios.post('users/cartDelect', {productId: that.delItem.productId}).then((response) => {
          let res = response.data;
          if (res.status == '0') {
            that.modalConfirm = false;
            that.initData();
