    ...
    methods: {
      initData() {
        ...
        axios.get('/users/cartList').then((response) => {
          ...
          that.cartList = res.result;
          that.cartList.forEach((item) => {
            if (item.checked == '1') {
              that.subTotal += item.salePrice * item.productNum;
            }
          });
          that.orderTotal = that.subTotal + that.shipping - this.discount + this.tax;
        })
      },
      payMent() {
        var that = this;
        ...
        axios.post('/users/payMent', {
          addressId: addressId,
          orderTotal: that.orderTotal
        }).then((response) => {
          let res = response.data;
          if (res.status == "0") {
            that.$router.push({path: 'orderSuccess', query: {orderId: res.result.orderId}});
          }
