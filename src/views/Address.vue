              ...
              <ul>
                <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}"
                  ...
                  <div class="addr-opration addr-set-default">
                    <a href="javascript:;" class="addr-set-default-btn" v-if="!item.default"
                       @click="setDefault(item.addressId)"><i>Set default</i></a>
                  </div>
                  <div class="addr-opration addr-default" v-if="item.default">Default address</div>
                </li>
                <li class="addr-new">
...
<script>
  ...
  export default {
    ...
    computed: {
      addressListFilter() {
        var that = this;
        return this.addressList.slice(0, that.limit);
      }
    },
    methods: {
      initAddress() {
        ...
        axios.get('/users/addressList').then((response) => {
          ...
          if (res.status == '0') {
            ...
            that.selectedAddrId = that.addressList[0].addressId;
          }
        })
      },
      expand() {
      ...
      delAddress() {
        ...
        axios.post('/users/delAddress',{
          ...
          let res = response.data;
          if(res.status == '0'){
            console.log('delect address success');
            that.isMdShow = false;
            that.initAddress();
          }
        })
